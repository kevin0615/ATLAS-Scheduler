...
public:
    ...
    list<Core_service> core_service_table; //Table of Attained Service

    typedef list<Request>::iterator ReqIter; 
    function<ReqIter(ReqIter, ReqIter)> least_Attained_Service =
    {   [this] (ReqIter req1, ReqIter req2){
        //if list of cores is empty add core
        //if (!core_service_table.size()) return req1;
        int rank1 = 0, rank2 = 0;
        //Go over list of cores and take the rank of each cores request
        for(typename list<Core_service>::iterator it = core_service_table.begin(); it!=core_service_table.end(); ++it){
            if(req1->coreid == it->coreid) rank1 = it->rank ;
            if (req2->coreid == it->coreid) rank2 = it->rank ;
        }
        //Return req of the core with the LAS
    ...
    void tick()
    {
        ...
        //Increment cycles of requests in readq and writeq for ATLAS
        for(list<Request>::iterator it = readq.q.begin(); it!=readq.q.end(); ++it)
            it->cycle += 1 ;    
        for(list<Request>::iterator it = writeq.q.begin(); it!=writeq.q.end(); ++it)
            it->cycle += 1 ;
        ...
        if (pending.size()) {
            ...
            if (req.depart <= clk) {
                ...
                update_core_service_table(pending[0]); 
                req.callback(req);
                pending.pop_front();
            }
        }
    ...
    //Add core to core service table and increment its AS
    int add_core_to_service_table(Request &req)
    { 
        Core_service newCore ;
        newCore.coreid = req.coreid;
        printf("%d\n", req.coreid);
        newCore.AS = 1;
        newCore.rank = 0;
        core_service_table.push_back(newCore);
    }

    //Update core service table
    int update_core_service_table(Request &req)
    {   
        //printf("7. IN\n");

        //if list of cores is empty add core
        if (!core_service_table.size()){

            //printf("8. List empty\n");

            add_core_to_service_table(req);
            return 0;
        }
        //printf("8. Core ID to be incremented : %d\n", req.coreid);
        //Go over list of cores and increment the AS
        for(typename list<Core_service>::iterator it = core_service_table.begin(); it!=core_service_table.end(); ++it){
            if(req.coreid == it->coreid){

            //    printf("9. Core ID to be incremented : %d\n", req.coreid);

                it->AS += 1;
                return 1;
            }
        }
        //if coreid missing, add it to the table
        add_core_to_service_table(req);

        //printf("10. Missing Core added\n");
