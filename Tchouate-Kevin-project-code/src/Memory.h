...
public:
    ...
    struct Total_AS{
        int coreid;
        long AS;
        long total_AS; //Total Attained Service
        int rank;
    };

     struct Core_service {
        int coreid;
        long AS; //Attained Service
        int rank;
    };
    
    list<Total_AS> total_Attained_Service_table; //Table of Total Attained Service
    ...
    //put AS of every core of total attained service table to 0
    void initialize_procedure(){
        for(typename list<Total_AS>::iterator it1 = total_Attained_Service_table.begin(); it1!=total_Attained_Service_table.end(); ++it1)
            it1->AS = 0;
    }

    //actualise total attained service table
    void update_TAS_table() {
        vector<long> ranking;
        for(typename list<Total_AS>::iterator it = total_Attained_Service_table.begin(); it!=total_Attained_Service_table.end(); ++it){
            it->total_AS = ALPHA * it->total_AS + (1-ALPHA) * it->AS;
            ranking.push_back(it->total_AS);
        }
        sort(ranking.begin(), ranking.end());
        int cmpt=0;
        for(typename vector<long>::iterator it = ranking.begin(); it!=ranking.end(); ++it){
            cmpt+=1;
            for(typename list<Total_AS>::iterator it1 = total_Attained_Service_table.begin(); it1!=total_Attained_Service_table.end(); ++it1){
                if (it1->total_AS == *it) it1->rank = cmpt;
            }
        }
    }
    ...
    void tick()
    {
        ...
        //if new quantum
        if(int_cycles % QUANTUM == 0){
            initialize_procedure();
            //Get Attained service of each coreid for the previous quantum
            for (auto ctrl : ctrls){
                bool pres; //presence
                //Go over list of cores and take the AS of each cores request
                for(typename list<typename Controller<T>::Core_service>::iterator it = ctrl->core_service_table.begin(); it!=ctrl->core_service_table.end(); ++it){
                    pres = false;
                    for(typename list<Total_AS>::iterator it1 = total_Attained_Service_table.begin(); it1!=total_Attained_Service_table.end(); ++it1){
                        //If coreid present update Total AS
                        if(it1->coreid == it->coreid){
                            it1->AS = it1->AS + it->AS;
                            pres=true;
                        }
                    }
                    //If coreid absent from total AS table add it
                    if(pres==false){
                        Total_AS newCore;
                        newCore.coreid = it->coreid;
                        newCore.AS = it->AS;
                        newCore.total_AS = 0;
                        total_Attained_Service_table.push_back(newCore);
                    }
                    //Reinitialize the AS of the core
                    it->AS = 0;
                }
            }
            //Update the total AS
            update_TAS_table();
            //actualier the total AS in each controller
            for (auto ctrl : ctrls){
                for(typename list<typename Controller<T>::Core_service>::iterator it = ctrl->core_service_table.begin(); it!=ctrl->core_service_table.end(); ++it){
                    for(typename list<Total_AS>::iterator it1 = total_Attained_Service_table.begin(); it1!=total_Attained_Service_table.end(); ++it1){
                        if(it1->coreid == it->coreid){
                ...
                //printf("Memory request coreid : %d\n", it->coreid);
            }
            int_cycles=0;
            printf("\n-----Start-----\n");
            for(typename list<Total_AS>::iterator it1 = total_Attained_Service_table.begin(); it1!=total_Attained_Service_table.end(); ++it1){
                    printf("\tCore ID : %d\ttotal_AS : %ld\n", it1->coreid, it1->total_AS);
            }
            printf("\n-----End-----\n");
        }
